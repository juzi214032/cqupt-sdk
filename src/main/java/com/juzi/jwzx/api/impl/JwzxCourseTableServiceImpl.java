...
import com.juzi.jwzx.api.JwzxCourseTableService;
 ...
 */
public class JwzxCourseTableServiceImpl implements JwzxCourseTableService {
    
    private JwzxService jwzxService;
    
    public JwzxCourseTableServiceImpl(JwzxService jwzxService) {
        this.jwzxService = jwzxService;
    }
    
    @Override
    public List<JwzxCourseTable> getStudentCourseTable(String studentId) {
    ...
    @Override
    public List<JwzxCourseTable> getTeacherCourseTable(String teacherId) {
    ...
    private List<JwzxCourseTable> parseCourseTable(Element tbody) {
        List<JwzxCourseTable> courseTableList = new ArrayList<>();
        // 课表所有行
        ...
        for (int i = 0; i < trs.size(); i++) {
            ...
            for (int j = 1; j < tds.size(); j++) {
                ...
                for (Element kb : kbs) {
                    ...
                    JwzxCourseTable jwzxCourseTable = new JwzxCourseTable();
                    ...
                    jwzxCourseTable
                            ...
                            .setCourseWeek(j);
                    // 存入集合
                    courseTableList.add(jwzxCourseTable);
                }
            }
            courseStartNo += 2;
        }
